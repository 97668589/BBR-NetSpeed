...
#2021.9.2 再次改为https://github.com/UJX6N/bbrplus-5.16
#2022.2.26 改为https://github.com/UJX6N/bbrplus-5.16

installbbrplusnew() {
  github_ver_plus=$(curl -s https://api.github.com/repos/UJX6N/bbrplus-5.16/releases | grep /bbrplus-5.16/releases/tag/ | head -1 | awk -F "[/]" '{print $8}' | awk -F "[\"]" '{print $1}')
  github_ver_plus_num=$(curl -s https://api.github.com/repos/UJX6N/bbrplus-5.16/releases | grep /bbrplus-5.16/releases/tag/ | head -1 | awk -F "[/]" '{print $8}' | awk -F "[\"]" '{print $1}' | awk -F "[-]" '{print $1}')
  echo -e "获取的UJX6N的bbrplus-5.16版本号为:${github_ver_plus}"
  echo -e "如果下载地址出错，可能当前正在更新，超过半天还是出错请反馈，大陆自行解决污染问题"
  ...
  if [[ "${release}" == "centos" ]]; then
    if [[ ${version} == "7" ]]; then
      if [[ ${bit} == "x86_64" ]]; then
        ...
        detele_kernel_head
        headurl=$(curl -s 'https://api.github.com/repos/UJX6N/bbrplus-5.16/releases' | grep ${github_ver_plus} | grep 'rpm' | grep 'headers' | grep 'el7' | awk -F '"' '{print $4}')
        imgurl=$(curl -s 'https://api.github.com/repos/UJX6N/bbrplus-5.16/releases' | grep ${github_ver_plus} | grep 'rpm' | grep -v 'devel' | grep -v 'headers' | grep -v 'Source' | grep 'el7' | awk -F '"' '{print $4}')
    ...
    if [[ ${version} == "8" ]]; then
      if [[ ${bit} == "x86_64" ]]; then
        ...
        kernel_version=${github_ver_plus_num}_bbrplus
        detele_kernel_head
        headurl=$(curl -s 'https://api.github.com/repos/UJX6N/bbrplus-5.16/releases' | grep ${github_ver_plus} | grep 'rpm' | grep 'headers' | grep 'el8' | awk -F '"' '{print $4}')
        imgurl=$(curl -s 'https://api.github.com/repos/UJX6N/bbrplus-5.16/releases' | grep ${github_ver_plus} | grep 'rpm' | grep -v 'devel' | grep -v 'headers' | grep -v 'Source' | grep 'el8' | awk -F '"' '{print $4}')
    ...
    if [[ ${bit} == "x86_64" ]]; then
      ...
      detele_kernel_head
      headurl=$(curl -s 'https://api.github.com/repos/UJX6N/bbrplus-5.16/releases' | grep ${github_ver_plus} | grep 'https' | grep 'amd64.deb' | grep 'headers' | awk -F '"' '{print $4}')
      imgurl=$(curl -s 'https://api.github.com/repos/UJX6N/bbrplus-5.16/releases' | grep ${github_ver_plus} | grep 'https' | grep 'amd64.deb' | grep 'image' | awk -F '"' '{print $4}')
...
start_menu() {
 ...
 ${Green_font_prefix}25.${Font_color_suffix} 安装 官方稳定内核                   ${Green_font_prefix}11.${Font_color_suffix} 使用BBR+FQ加速
 ${Green_font_prefix}26.${Font_color_suffix} 安装 Zen官方内核                    ${Green_font_prefix}12.${Font_color_suffix} 使用BBR+FQ_PIE加速
 ...
 ${Green_font_prefix}安装:${Font_color_suffix} BBR+BBRplus+Lotserver             ${Green_font_prefix}16.${Font_color_suffix} 使用BBR2+CAKE加速
 ...
 ${Green_font_prefix}4.${Font_color_suffix} 安装 Lotserver(锐速)内核             ${Green_font_prefix}20.${Font_color_suffix} 开启ECN
 ${Green_font_prefix}5.${Font_color_suffix} 查看排序内核                         ${Green_font_prefix}21.${Font_color_suffix} 禁用IPv6
 ${Green_font_prefix}6.${Font_color_suffix} 删除保留指定内核                     ${Green_font_prefix}22.${Font_color_suffix} 开启IPv6
 ${Green_font_prefix}7.${Font_color_suffix} 切换到一键DD系统脚本                 ${Green_font_prefix}23.${Font_color_suffix} 系统配置优化
 ${Green_font_prefix}8.${Font_color_suffix} 切换到卸载内核版本                   ${Green_font_prefix}24.${Font_color_suffix} 应用johnrosen1的优化方案
